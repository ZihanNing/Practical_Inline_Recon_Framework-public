...
if ~rec.Alg.WriteRawY;rec.Dyn.Typ2Rec(ismember(rec.Dyn.Typ2Rec,[3 6]))=[];else rec.Dyn.Typ2Rec(ismember(rec.Dyn.Typ2Rec,3))=[];end
...
if ~rec.Par.Mine.Proce
    ...
    if rec.Alg.EstimGFact(1) && isfield(rec,'G') && isfield(rec,'M') && rec.Dyn.Typ2Wri(9)
        ...
        S=dynInd(S,1,6);
        if size(M,4)>size(rec.G,4)
            fprintf('More volumes in the mask than in the g-factor for %s\n',rec.Names.Name);
...
if isfield(rec,'MT')%Case procePipelineTransform
    MT=rec.MT{1};
    MS=rec.MS{1};
    if ismember(rec.Par.Mine.Modal,9:10);MS(4)=rec.Par.Labels.RepetitionTime(1)/1000;else MS(4)=1;end
else
...
for n=typ2Rec';datTyp=rec.Plan.Types{n};datName=rec.Plan.TypeNames{n};    
    if rec.Dyn.Typ2Wri(n)
        ...
        if ~ismember(n,24:25) && isfield(rec,'Enc') && isfield(rec.Enc,'ROI');MTV{c}=MTROI;else MTV{c}=MT;end
...
if c>1;writenii(fileName,x,suff,MSV,MTV);end
