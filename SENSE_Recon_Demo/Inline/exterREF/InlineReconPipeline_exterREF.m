function rec=InlineReconPipeline_exterREF(twix_like)
...
% if isempty(rec.Nam.bodyIn)
    n0=3; % target scan with ACS, process ACS first as the reference, then the imaging data with SENSE recon
...
%     [rec,cont]=reconRead(rec,n,twix_like);    
    ...
    %INVERT
    tsta=tic;
%     recon_flexFOV = [224, 250, 48]; % cannot be used yet
    rec=reconInvert_inline(rec,n,'image',[]);
    fprintf('Time inverting %s: %.2f\n',typV(n),toc(tsta));
    
    %LOAD EXTER_REF
    recS_path = loadLatestDependency(twix_like.hdr.save_path, 'ExterREF_CMS');
    load(recS_path)
    rec.S = recS.S; rec.xS = recS.xS; rec.B = recS.B;
    rec.Geom.S = recS.Geom.S; 
    recS = []; % free memory
