...
if nargin<4 || isempty(ichk);ichk=0;end
...
if i_type~=4 && o_type~=4,  %if input/output are NOT Euler angles
    ...
    if i_type==4,   %if input type is Euler angles, determine the order of rotations
        ...
        if rot_1_in==rot_3_in % Type 2 rotation (1st and 3rd rotations about same axis) 
            ...
            %INPUT(EA2>90,2)=INPUT(EA2>90,2)-360;
            %EA2(EA2>90)=EA2(EA2>90)-360;%My modification
            %INPUT(EA2<-90,2)=INPUT(EA2<-90,2)+360;%My modification
            %EA2(EA2<-90)=EA2(EA2<-90)+360;%My modification
...
switch INPUT_TYPE
    ...
    case 'EV'  %Euler Vector Input Type
        ...
        if any(MU<zeros(N,1) | MU>2*pi*ones(N,1)), %check if rotation about euler vector is between 0 and 360
...
switch OUTPUT_TYPE
    ...
    case 'EA'
	  ...
	  Euler_type=1;
        ...
        if Euler_type==1,
            ouo=find(OUTPUT(:,2)>180);%MY MODIFICATION
            OUTPUT(ouo,2)=OUTPUT(ouo,2)-360;%MY MODIFICATION
	        sing_chk=find(abs(theta)*180/pi>89.999);%IT WAS 89.0 BEFORE BUT IT WAS PRODUCING ERRORS SOME TIMES, SO I'VE CHANGED IT            
        ...
        elseif Euler_type==2,
	        sing_chk=[find(abs(theta*180/pi)<0.1);find(abs(theta*180/pi-180)<0.1);find(abs(theta*180/pi-360))<0.001];%IT WAS 0 BEFORE, BUT ERRORS AGAIN, SO CHANGED
	        sing_chk=sort(sing_chk(sing_chk>0));
	        if size(sing_chk,1)>=1,
