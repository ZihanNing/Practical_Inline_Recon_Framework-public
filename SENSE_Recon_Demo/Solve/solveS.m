...
if useGPU;rec.y=gpuArray(rec.y);rec.x=gpuArray(rec.x);end
...
if ~isfield(rec.Alg,'parS');rec.Alg.parS=[];end
parS=rec.Alg.parS;

%PARAMETERS FOR COIL AND MASK ESTIMATION
if ~isfield(parS,'maskNorm');parS.maskNorm=1;end%Norm of the body coil intensities for mask extraction%It was 2
if ~isfield(parS,'maskTh');parS.maskTh=0.2;end%Threshold of the body coil intensities for mask extraction%It was 0.2
if ~isfield(parS,'Otsu');parS.Otsu=[0 1];end%Binary vector of components for multilevel extraction (it picks those components with 1)    
if ~isfield(parS,'lambda');parS.lambda=20;end%This parameter has been constantly changing, toddlers have been processed with it set to 1, but it was introducing noise for neonates, 2 may suffice, but 5 gives a bit of margin
if ~isfield(parS,'order');parS.order=2;end%Regularization order for coil estimation-Something around 0.2 is optimal for order 2
if ~isfield(parS,'nErode');parS.nErode=0;end%2;%Erosion for masking (in mm)
if ~isfield(parS,'nDilate');parS.nDilate=3;end%Dilation for masking (in voxels then in mm)
if ~isfield(parS,'conComp');parS.conComp=2;end%Whether to get the largest connected component after erosion (1) and to fill holes (2)
if ~isfield(parS,'GibbsRingi');parS.GibbsRingi=[0 0];end%Gibbs ringing for coil profiles
if ~isfield(parS,'ResolRatio');parS.ResolRatio=[1 1];end%Resolution ratio for coil profiles
if ~isfield(parS,'TolerSolve');parS.TolerSolve=1e-5;end%S-solver tolerance
if ~isfield(parS,'nIt');parS.nIt=300;end%S-solver maximum number of iterations
if ~isfield(parS,'softMaskFactor');parS.softMaskFactor=10;end%Factor to normalize the body coil data for soft masking

if ~isfield(rec,'Plan') || ~isfield(rec.Plan,'NDims');rec.Plan.NDims=14;end
if ~isfield(rec,'Dyn') || ~isfield(rec.Dyn,'Debug');rec.Dyn.Debug=1;end
if ~isfield(rec,'Par') || ~isfield(rec.Par,'Labels') || ~isfield(rec.Par.Labels,'CoilNrsPerStack');rec.Par.Labels.CoilNrsPerStack=[];end
...
if NPE==1
    D=median(D,5:rec.Plan.NDims);
    for s=1:N(4);Z=dynInd(Z,[s 1],4:5,median(dynInd(Z,s,4),5:rec.Plan.NDims));end
    Z=dynInd(Z,1,5);
end
for p=1:NPE
    ...
    %%%THERE WAS A BUG IN PREVIOUS LINE: DC COMPONENT WAS NOT CERO, I'VE CHECKED THAT THIS IS NOT HAVING AN IMPACT ON FETUSES, NEEDS TO BE CHECKED IN NEONATES
...
rec.M=gather(rec.M);rec.S=gather(rec.S);
