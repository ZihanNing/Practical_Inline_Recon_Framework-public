...
if ~isempty(E)
    ...
    %WE FORCE THIS FOR MEMORY
        E.bS=[1 1];E.dS=[size(x,5) size(x,4)];E.oS=[1 1];
        if isfield(E,'Sf');E.bS(2)=1;E.dS(2)=size(E.Sf,4);end
        if isfield(E,'Tr');E.bS(1)=1;E.dS(1)=E.NSe;end
    %end
end
if ~isempty(EH)
    if isfield(EH,'We')
            ...
            if isfield(E,'Uf')%Sense folding (first two dimensions)
                for n=1:2
                    if ~isempty(E.Uf{n})
                        ...
                        if E.Uf{n}.NX<E.Uf{n}.NY
                            NXS=size(xS);NXS(n)=E.Uf{n}.NY;
                            xS=resampling(xS,NXS,3);
                        end
                    end
                end
            end
