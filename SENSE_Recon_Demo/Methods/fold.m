function x=fold(x,m,NS,NY,A,fl,fo)
...
%   * {FO} serves to apply Fourier folding, it defaults to 0, with 2 it
%   applies folding shifted (for MB)
...
if NS>NY%Otherwise it has no action
    ...
    if mod(NS,2)==1 && mod(NY,2)==0;FOV=[ceil(over) floor(over)];else FOV=[floor(over) ceil(over)];end
