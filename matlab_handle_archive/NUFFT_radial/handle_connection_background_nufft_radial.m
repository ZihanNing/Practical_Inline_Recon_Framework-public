...
% ---------------------------------------------------------------
    ...
    supportPaths = addSupportPath(Recon_ID); % Add the path of the custom recon (the path set in Framework_config.xml in the field <support_func_path>)
    ...
    try
        ...
        config.seq_type = 'radial';
        
        % save the image for later retrieval (could be multiple of images)
        % img 1
        config.fileName = ['arrayimg1_sos_',twix_like.hdr.fileName]; % prevent overwriting
        [fileToSend, status] = save_for_retrieval(arrayimg1_sos,twix_like,config,1); 
        % for non-cartesian seq, the scanner cannot provide conventional
        % recon, therefore the meta is not generated here with the image to
        % be retireved, instead, it will be generated during retrieval by
        % the retrieval dummy seq (Cartesian & align with the recon img)
