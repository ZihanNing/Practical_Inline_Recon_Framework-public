...
% ---------------------------------------------------------------
    ...
    try
        ...
        rec=InlineRecon_ExterREF_ESPIRIT(twix_like);
        
        %% Save the dependencies (e.g., cms) for later use
        save_path = twix_like.hdr.save_path;
        fileName = ['ExterREF_CMS_',twix_like.hdr.fileName,'.mat'];
        recS = rec; save([save_path,'/',fileName],'recS','-v7.3');
        
        %% Save the image to be retrieved
        % for the external REF, we do not retrieve any image to the scanner
        % (just keep the scanner-reconstructed exterREF)
        status = 1;
