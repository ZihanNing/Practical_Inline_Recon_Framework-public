...
parXT=rec.Alg.parXT;
gpu=rec.Dyn.GPU;gpuIn=useGPU;if gpuIn;gpuFIn=2;else gpuFIn=0;end
...
if ~isempty(rec.Assign.Shots)
    NShots=max(rec.Assign.Shots);
    NEchos=accumarray(rec.Assign.Shots',ones(length(rec.Assign.Shots),1))';   
    if rec.Dyn.Debug>=2    
        fprintf('Real number of shots: %d\n',NShots);
    end
end
...
if isSteadyState
    ...
    NSweeps=ceil(NProfs/(NSamples*parXT.groupSweeps));%Recent change
...
if isempty(rec.Assign.Shots);sweepSample=ceil(NSweeps*(((1:NProfs)-0.5)/NProfs));
else sweepSample=rec.Assign.Shots;
end
...
if isSteadyState
    ...
    if ~isempty(rec.Assign.TimeStamp)
        timeSample=rec.Assign.TimeStamp;
    elseif ~isempty(rec.Assign.Shots)
        error('If shots are predefined, one should also give time stamp')
    else
...
for l=1:L    
    ...
    %%%THERE WAS A BUG IN PREVIOUS LINE: DC COMPONENT WAS NOT CERO, IT 
    %%%SHOULD WORK SLIGHTLY BETTER NOW
    ...
    if any(nSa(1:NStates)==0)
        nowithin=1;
        fprintf('Probably not DISORDER, returning without performing corrections\n');
        rec.Dyn.Typ2Rec(ismember(rec.Dyn.Typ2Rec,[16 18]))=[];
        rec.Dyn.Typ2Wri([16 18])=0;

        %INVERT PERMUTATION OF PHASE ENCODES
        typ2Rec=rec.Dyn.Typ2Rec;
        for n=typ2Rec';datTyp=rec.Plan.Types{n};
            if ~ismember(n,5);rec.(datTyp)=ipermute(rec.(datTyp),perm);end
        end
        %INVERT EXTRACTION OF ROI
        for n=typ2Rec';datTyp=rec.Plan.Types{n};
            if ~ismember(n,5);rec.(datTyp)=extractROI(rec.(datTyp),rec.Enc.ROI,0,1);end
        end
        %POSTPROCESSING
        for n=12;datTyp=rec.Plan.Types{n};
    ...
    for n=1:nExtern
        ...
        if rec.Alg.WriteSnapshots && estT(l)==0;visResiduals(We,outlWe,timeSweep,resampling(Residuals,NY(1:2),4),kTraj,2,{strcat(folderSnapshots,filesep,'ResidualsStates'),strcat(folderSnapshots,filesep,'ResidualsSpectrum')},strcat(fileName,rec.Plan.SuffOu,sprintf('_l=%d',l)));end
