...
if nargin<7 || isempty(complex);complex=0;end
...
for j=1:J;BP(:,:,j)=resampling(FHP{j},N,3);end
BP=fft2D(BP);
...
for k=unique(K)
    ...
    WDups=upsampling(resampling(conv2(DFups,FLP2{Kmax-k}'),N,3),2,2^k);  
    
    %CONVOLVE WEDGE FILTER WITH LOWPASS FILTER, AGAIN FOLLOWING EQ (21) ON 
    %PAG 14.
    FLPaux=resampling(FLP2{Kmax-max(k-1,0)},size(WDups),3);    
...
LP=fft2D(resampling(FLP1{1}'*FLP1{1},N,3));    
