function x=ifold(x,m,NS,NY,A,fl,fo)
...
%   * {FO} serves to apply Fourier inverse folding, it defaults to 0, with
%   2 it applies folding shifted (for MB)
%   ** X is the unfolded array
%

if nargin<5;A=[];end
if nargin<6 || isempty(fl);fl=0;end
if nargin<7 || isempty(fo);fo=0;end

if NS>NY%Otherwise it has no action
    if fo==1;x=fftGPU(x,m);end
    if fo;x=fftshift(x,m);end    
    ND=ndims(x);
    oddFactSENSE=2*ceil((ceil(NS/NY)-1)/2)+1;
    oFRed=oddFactSENSE-2;
    disc=(oddFactSENSE*NY-NS)/2;
    %This if is a recent modification over Debug07
    if mod(NS,2)==1 && mod(NY,2)==0;iFOV=[ceil(disc) floor(disc)];else iFOV=[floor(disc) ceil(disc)];end
    ...
    if fo;x=ifftshift(x,m);end
    if fo==1;x=ifftGPU(x,m);end  
end
