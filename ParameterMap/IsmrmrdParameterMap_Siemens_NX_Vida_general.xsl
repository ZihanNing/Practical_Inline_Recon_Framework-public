...
<xsl:stylesheet version="1.0"
    ...
    <!--ZN: the following method to extract phaseOS failed for NX60-->
    ...
    <!--Zihan: the following method works to extract phaseOS for NX60 version, start-->
    <xsl:variable name="phaseOversampling">
        <xsl:choose>
            <xsl:when test="not(siemens/DICOM/flPhaseOS)">0</xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="string(number(siemens/DICOM/flPhaseOS)) = 'NaN'">0
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="siemens/DICOM/flPhaseOS"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    <!--Zihan: the following method works to extract phaseOS for NX60 version, end-->
    ...
    <xsl:template match="/">
        <ismrmrdHeader xsi:schemaLocation="http://www.ismrm.org/ISMRMRD ismrmrd.xsd"
            ...
            <!-- Not anonymised, for gadgetron processing and result saving, start -->
            <subjectInformation>
                <patientName>
            ...
            <!-- Not anonymised, for gadgetron processing and result saving, end -->
            ...
            <encoding>
                ...
                <encodedSpace>
                    <matrixSize>
                        ...
                        <xsl:when test="siemens/MEAS/sKSpace/ucTrajectory = 1">
                            <x>
                                <xsl:value-of select="floor(siemens/YAPS/iNoOfFourierColumns)"/>
                            </x> 
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:choose>
                ...
                <reconSpace>
                    <matrixSize>
                        ...
                        <xsl:otherwise>
                            <y>
                                <!--Zihan: I don't think that the recon FOVmm contains the phaseoversampling, since the dPhaseFOV doesn't contains PE OS-->
                                <!--<xsl:value-of select="floor(siemens/MEAS/sSliceArray/asSlice/s0/dPhaseFOV * (1+$phaseOversampling) div $pixelSpacing + 0.5)"/>-->
                                <xsl:value-of select="floor(siemens/MEAS/sSliceArray/asSlice/s0/dPhaseFOV div $pixelSpacing + 0.5)"/> <!--Zihan modified-->
                            </y>
                        </xsl:otherwise>
                        </xsl:choose>
                ...
                <encodingLimits>
                    <kspace_encoding_step_1>
                        ...
                        <xsl:choose> <!-- >>> ZN: for PF in PE1 direction >>> -->
                            <xsl:when test="siemens/MEAS/sKSpace/ucPhasePartialFourier > 8"> <!-- PF isn't applied on PE1 direction -->
                                <center>
                                    <xsl:value-of select="floor(siemens/MEAS/sKSpace/lPhaseEncodingLines div 2)"/> 
                                </center>
                            </xsl:when>
                            <xsl:otherwise>
                                <center>  <!-- when PF is applied -->
                                    <xsl:value-of select="ceiling((siemens/MEAS/sKSpace/lPhaseEncodingLines + 1) div 2) - (siemens/MEAS/sKSpace/lPhaseEncodingLines - siemens/YAPS/iNoOfFourierLines)"/> 
                                </center>
                            </xsl:otherwise>
                        </xsl:choose> <!-- <<< ZN: for PF in PE1 direction <<< -->
        ...
        <parallelImaging>
          ...
          <xsl:if test="(siemens/MEAS/sPat/ucRefScanMode = 1) or (siemens/MEAS/sPat/ucRefScanMode = 16) or (siemens/MEAS/sPat/ucRefScanMode = 32) or (siemens/MEAS/sPat/ucRefScanMode = 64)">
            ...
            <sequenceParameters>
                ...
                <!-- This has been modified by zihan start-->
                ...
                <xsl:if test="siemens/DICOM/adFlipAngleDegree">
                    <flipAngle_deg>
                        <xsl:value-of select="siemens/DICOM/adFlipAngleDegree" />
                    </flipAngle_deg>
                </xsl:if>
                <!-- This has been modified by zihan end-->
            ...
            <userParameters>
                ...
                <xsl:if test="siemens/MEAS/lScanTimeSec">
                    <userParameterLong>
                        ...
                        <value>
                            <xsl:value-of select="siemens/MEAS/lScanTimeSec" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <xsl:if test="siemens/MEAS/sKSpace/unReordering">
                    <userParameterLong>
                        <name>unReordering_zihan</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sKSpace/unReordering" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <userParameterLong>
                    <name>lBaseResolution_zihan</name>
                    <value>
                        <xsl:value-of select="siemens/MEAS/sKSpace/lBaseResolution" />
                    </value>
                </userParameterLong>
                
                <xsl:if test="siemens/DICOM/lGlobalTablePosTra">
                    <userParameterLong>
                        <name>lGlobalTablePosTra_zihan</name>
                        <value>
                ...
                <xsl:if test="siemens/MEAS/sKSpace/ucEnableEllipticalScanning">
                    <userParameterLong>
                        <name>ucEnableEllipticalScanning</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sKSpace/ucEnableEllipticalScanning" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                ...
                <xsl:if test="siemens/DICOM/ucPATMode">
                    <userParameterLong>
                        <name>ucPATMode</name>
                        <value>
                            <xsl:value-of select="siemens/DICOM/ucPATMode" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <xsl:if test="siemens/MEAS/sPAT/ucRefScanMode">
                    <userParameterLong>
                        <name>ucRefScanMode</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sPAT/ucRefScanMode" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <!--tse seq related-->
                <xsl:if test="siemens/MEAS/sFastImaging/lTurboFactor">
                    <userParameterLong>
                        <name>lTurboFactor</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sFastImaging/lTurboFactor" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <xsl:if test="siemens/MEAS/sFastImaging/lEchoTrainDuration">
                    <userParameterLong>
                        <name>lEchoTrainDuration</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sFastImaging/lEchoTrainDuration" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <xsl:if test="siemens/DICOM/RFEchoTrainLength">
                    <userParameterLong>
                        <name>RFEchoTrainLength</name>
                        <value>
                            <xsl:value-of select="siemens/DICOM/RFEchoTrainLength" />
                        </value>
                    </userParameterLong>
                </xsl:if>
                
                <!--WIP parameters: user defined in the sequence-->
                <xsl:if test="siemens/MEAS/sWipMemBlock/alFree[2]">
                      <!-- Test if both parameters exist -->
                      <userParameterLong>
                            <name>sWipMemBlock_alFree_2</name>
                            <value>
                              <xsl:value-of select="siemens/MEAS/sWipMemBlock/alFree[2]"/>
                            </value>
                      </userParameterLong>
                </xsl:if>
                
                <xsl:if test="siemens/MEAS/sWipMemBlock/alFree[3]">
                      <!-- Test if both parameters exist -->
                      <userParameterLong>
                            <name>sWipMemBlock_alFree_3</name>
                            <value>
                              <xsl:value-of select="siemens/MEAS/sWipMemBlock/alFree[3]"/>
                            </value>
                      </userParameterLong>
                </xsl:if>
                ...
                <!--Add user parameters here, start-->
                <!--Added by Zihan-->
                
                <xsl:if test="siemens/DICOM/flReadoutOSFactor">
                      <userParameterDouble>
                        <name>flReadoutOSFactor</name>
                        <value>
                            <xsl:value-of select="siemens/DICOM/flReadoutOSFactor" />
                        </value>
                      </userParameterDouble>
                  </xsl:if>
                  ...
                  <xsl:if test="siemens/DICOM/flSliceOS">
                      <userParameterDouble>
                        <name>flSliceOS_zihan</name>
                        <value>
                            <xsl:value-of select="siemens/DICOM/flSliceOS" />
                        </value>
                      </userParameterDouble>
                  </xsl:if>
                  ...
                  <xsl:if test="siemens/MEAS/sKSpace/dPhaseResolution">
                      <userParameterDouble>
                        <name>dPhaseResolution_zihan</name>
                        <value>
                  ...
                  <xsl:if test="siemens/MEAS/sRXSPEC/alDwellTime">
                      <userParameterDouble>
                        <name>alDwellTime_zihan</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sRXSPEC/alDwellTime" />
                        </value>
                      </userParameterDouble>
                  </xsl:if>
                  ...
                  <xsl:if test="siemens/DICOM/dSliceResolution">
                      <userParameterDouble>
                        <name>dSliceResolution_zihan</name>
                        <value>
                            <xsl:value-of select="siemens/DICOM/dSliceResolution" />
                        </value>
                      </userParameterDouble>
                  </xsl:if>
                  
                  <xsl:if test="siemens/MEAS/sSliceArray/asSlice/s0/dReadoutFOV">
                      <userParameterDouble>
                        <name>dReadoutFOV_zihan</name>
                        <value>
                            <xsl:value-of select="siemens/MEAS/sSliceArray/asSlice/s0/dReadoutFOV" />
                        </value>
                      </userParameterDouble>
                  ...
                  <xsl:if test="siemens/YAPS/flReadoutOSFactor">
                      <userParameterDouble>
                        <name>flReadoutOSFactor_zihan</name>
                        <value>
                ...
                <!--Add user parameters here, start-->
                <!--Added by Zihan-->
                <xsl:if test="siemens/DICOM/tScanningSequence">
                    <userParameterString>
