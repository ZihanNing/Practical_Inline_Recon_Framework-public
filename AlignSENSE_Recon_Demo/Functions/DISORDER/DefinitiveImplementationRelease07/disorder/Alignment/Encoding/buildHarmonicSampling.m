...
NH=size(H);H(end+1:3)=1; % YB: should be NH(end+1:3) = 1; I think
H=H(:);
...
FSt=cell(2,NStates+1);ySt=cell(1,NStates+1);GSt=cell(1,NStates+1);iStInd=cell(1,NStates+1);mSt=H(:);%YB: +1 comes from the area outside elliptic shutter
