function [xRes,SRes,yRes,ARes,MRes,rGridRes,kGridRes,rkGridRes,F,FH,fail]=downsampleOperators(l,x,S,y,A,M,gibbsRing,fo,gpuIn,useFilter)
...
%   * {USEFILTER} to control the use a filter after the resampling (to  avoid ringing) (Added by YB for simulations - don't want to alter data)
...
if nargin<10 || isempty(useFilter);useFilter=1;end

gpu=isa(x,'gpuArray');
if nargin<9 || isempty(gpuIn);gpuIn=gpu;end

NX=size(x);NY=size(y);NX(end+1:3)=1;NY(end+1:12)=1;%YB: This 12 probably comes from the ND=12 and shoud not be hardcoded
NXRes=ceil(NX(1:3)./(2.^l));NYRes=ceil(NY(1:3)./(2.^l));%YB: NXRes and NYRes can have different dimensions in case of under-sampling
...
if useFilter==1 && gibbsRing~=0 && gibbsRing<=1;HY=buildFilter(NYRes(1:3),'tukeyIso',[],gpuIn,gibbsRing);elseif useFilter==1 & l~=0;HY=buildFilter(NYRes,'CubicBSpline',[],gpuIn);else HY=[];end
if ~isempty(HY)
