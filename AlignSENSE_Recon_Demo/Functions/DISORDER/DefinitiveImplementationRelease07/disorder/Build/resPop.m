...
x = gather(x); % ZN: to solve out of memory issue or potentiall iostream issue
x=permute(x,perm);
...
x=permute(x,perm);
gpuInfo = gpuDevice; matrixSize = whos('x'); 
if gpuInfo.AvailableMemory > matrixSize.bytes *5; x=gpuArray(x);end % ZN: load the x back if there's enough GPU memory to load it back
N=size(x);
