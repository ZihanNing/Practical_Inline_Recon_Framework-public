...
for n=1:max(nIt)  
    ...
    if ~isfield(E,'Je') || E.Je==0;EHE=constrain(decode(encode(p,E),EH,E)+regularize(p,R),C);else EHE=constrain(encodedecode(p,E,EH)+regularize(p,R),C);end %YB: Je = 'joint encoding'
...
if estGF(1)>0
    if deb>=2;fprintf('Number of iterations for g-factor calculations: %d\n',nn);end
