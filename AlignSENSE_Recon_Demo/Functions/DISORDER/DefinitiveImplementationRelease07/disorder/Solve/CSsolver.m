...
if isfield(E,'corrFact'); corrFact = E.corrFact; else corrFact =1;end
gpu=isa(x,'gpuArray');
...
for n=1:nOu
    ...
    if n~=nOu
        computeShearlet;
        %COMPUTE SHEARLETS
        R.Sh.We=(R.Sh.We+epsil).^(R.Sh.p-2);%YB: This is taking the inverse
    
        %COMPUTE NEW LAMBDA
        trReg=traceEstimation(x,[],[],C,R,1e-2,4);
        R.Sh.la=(2/R.Sh.p)*trFid/(trReg+eps);
        lambda(n+1)=corrFact*R.Sh.la;
...
function computeShearlet
    ...
    for w=1:NW
        ...
        R.Sh.We(:,:,:,w)=gather(abs(sh));%YB: Here you have the abs that is in my book for W_ii
    end
end
