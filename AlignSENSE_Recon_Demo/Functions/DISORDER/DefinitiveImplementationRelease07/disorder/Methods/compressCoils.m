...
%                     if gpuIn;SF=gpuArray(SF);end
                    ...
                    BlSz=1e5;
                    matrixSize = whos('SF'); gpuInfo = gpuDevice;
                    if gpuInfo.AvailableMemory > matrixSize.bytes *5; if gpuIn;SF=gpuArray(SF);end; end % ZN: if GPU has enough room to load the matrix 'SF' then load it all at once
                    for p=1:BlSz:size(SF,3);vn=p:min(p+BlSz-1,size(SF,3));
                        SFaux=dynInd(SF,vn,3);
                        if ~isa(SF,'gpuArray');SFaux = gpuArray(SFaux);end
    ...
    if ~isempty(y)
        ...
        NN=size(y,5);
        y=gather(y);
        yo=zeros([NY(1:3) size(A,1) NN],'like',y);
        for n=1:NN
            yn=dynInd(y,n,5);
            if gpu;yn=gpuArray(yn);end
            yn=gather(aplGPU(A,yn,4));
            yo=dynInd(yo,n,5,yn);
        end
        y=yo;yo=[];
        if gpu;y=gpuArray(y);end
        y=reshape(y,[NY(1:3) size(A,1) NY(5:end)]);
    end%Compress data
