...
function rec=solveXTBv0(rec)

%rec.Alg.WriteSnapshots=0;
%rec.Alg.parXT.exploreMemory=1;
%rec.Alg.AlignedRec=2;
%if rec.Alg.AlignedRec==2;rec.Alg.parXT.NWend=16;rec.Alg.parXT.accel=[0 0];end%Full within-shot corrections
...
if rec.Dyn.Log
    ...
    logName =  strcat( logDir, fileName, '.txt'); 
...
if parXB.dephaseCorrection>0;typ2RecF=29;else typ2RecF=[];end 
for n=typ2RecI'
...
for l=1:L    
    ...
    for n=1:nExtern
        ...
        [E.Tr,E.Fs,E.nEc]=compressMotion(rec.T,FSt,resIso(end),parXT);%YB: E.nEc are the starting indiced - used in the reconstruction (decode.m)
        ...
        if nX~=0 %YB: this is where you are actually solving for X and B0
            if estT(l) && estB(l) && parXB.dephaseCorrection>0 && any(E.Tr(:)~=0)
                if ~isfield(E,'Dc')
                    fprintf('Joint dephasing and signal estimation\n');
                    fprintf('Size of reconstruction grid:%s\n',sprintf(' %d',NX));
                    %INITIALIZE
                    NXresA=NXres;NXresA(parXB.DCTdims==0)=1;
                    NXresB=NXres;NXresB(parXB.DCTdims==1)=1;
                    E.Dc.NX=NXresB;%Fully encoded dimensions
                    E.Dc.A=extractDCTAtoms(NXresA,parXB.dephaseCorrection); % YB: dephaseCorrection is the number of DCT atoms
                    E.Dc.A=resPop(E.Dc.A,4,[],7);
                    if strcmp(rec.Par.Labels.FatShiftDir,'H') || strcmp(rec.Par.Labels.FatShiftDir,'F');E.Dc.d=5:6;
                    elseif strcmp(rec.Par.Labels.FoldOverDir,'FH') || strcmp(rec.Par.Labels.FoldOverDir,'HF');E.Dc.d=[4 6];
                    else E.Dc.d=4:5;
                    end
                    fprintf('In-plane rotations:%s\n',sprintf(' %d',E.Dc.d));

                    E.Dc.c=zeros([on{5} 2 parXB.dephaseCorrection],'like',E.Dc.A);
                    E.Dc.c=repmat(E.Dc.c,NXresB);

                    E.Dc.D=sum(bsxfun(@times,E.Dc.c,E.Dc.A),7);%Residual phases per unit axial rotation
                    E.Dc.winit=parXB.winit;
                    E.Dc.w=E.Dc.winit*ones(E.Dc.NX);
                    E.Dc.cL=parXB.tolerSolve;
                    E.Dc.cC=zeros(E.Dc.NX);%Convergence flag
                    %SOLVE---NOTE THAT THE PARAMETERS DO NOT CONNECT WELL
                    %BETWEEN RESOLUTIONS, SO THIS IS DONE ONCE AT THE LAST
                    %RESOLUTION WITH MOTION ESTIMATES
                end
                xRes=CGsolver(yX,E,EH,P,CX,R,xRes,nX,tolType{estT(l)+1},tol(estT(l)+1));           
                E.Dc.cC(:)=0; 
                E=LMsolverDephasing(yX,E,xRes);          
            end
            [xRes,EnX]=CGsolver(yX,E,EH,P,CX,R,xRes,nX,tolType{estT(l)+1},tol(estT(l)+1));           
        end
plot_(abs(xRes));
        if ~isempty(EnX)%Print and store final energy
            EnX=EnX/numel(yX);
        ...
        if leff>0 && estT(l)==0
            ...
            if parXT.computeCSRecon
                %save('/home/lcg13/Work/DataDefinitiveImplementationDebug06/recTSE.mat','yX','E','EH','P','CX','R','xRes','nX','tolType','estT','l','tol','-v7.3');   
            ...
            EH.We=[];%YB: Weigting only in combination with CSsolver, currenyly, you cannot apply weights and shearlets independently.
            
            if parXB.dephaseCorrection>0 && ~parXT.exploreMemory %YB: added  && ~parXT.exploreMemory
                rec.D=gather(E.Dc.D);            
                for t=typ2RecF'
        ...
        if estT(l) % YB: This is where you actually update the motion parameters              
            %LM SOLVER        
    ...
    if ~parXT.exploreMemory && isfield(E,'Dc') && isfield(E.Dc,'D') && l~=L ;DB=resampling(E.Dc.D,NX,0,2*ones(1,3));end %Y: parXT.exploreMemory added 
