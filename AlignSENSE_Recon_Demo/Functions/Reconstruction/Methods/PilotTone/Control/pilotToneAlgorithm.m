...
if ~isfield(rec.Alg.parXT.PT,'subDiv');rec.Alg.parXT.PT.subDiv=0;end 
...
if ~isfield(rec.Alg.parXT.PT.signalUsage,'normaliseCoils');rec.Alg.parXT.PT.signalUsage.normaliseCoils=0;end%To normalise the signal have unit norm across the difference coils
if ~isfield(rec.Alg.parXT.PT.signalUsage,'whiten');rec.Alg.parXT.PT.signalUsage.whiten=1;end%Make phase of each coil have zero mean to avoid phase wrapping ambiguities.
if ~isfield(rec.Alg.parXT.PT.signalUsage,'whitenPhase');rec.Alg.parXT.PT.signalUsage.whitenPhase=1;end%Make phase of each coil have zero mean to avoid phase wrapping ambiguities.
if ~isfield(rec.Alg.parXT.PT.signalUsage,'useMagn');rec.Alg.parXT.PT.signalUsage.useMagn=0;end%To use magnitude of PT signal.
if ~isfield(rec.Alg.parXT.PT.signalUsage,'usePhase');rec.Alg.parXT.PT.signalUsage.usePhase=0;end%To use phase of PT signal.
if ~isfield(rec.Alg.parXT.PT.signalUsage,'useRealImag');rec.Alg.parXT.PT.signalUsage.useRealImag=0;end%Concatenate real and imaginary signal with virtually 2*NCoils.
if ~isfield(rec.Alg.parXT.PT.signalUsage,'useComplex');rec.Alg.parXT.PT.signalUsage.useComplex=~(rec.Alg.parXT.PT.signalUsage.useMagn || rec.Alg.parXT.PT.signalUsage.usePhase || rec.Alg.parXT.PT.signalUsage.useRealImag);end%To use the complex PT signal.
assert(multDimSum([rec.Alg.parXT.PT.signalUsage.useMagn rec.Alg.parXT.PT.signalUsage.usePhase rec.Alg.parXT.PT.signalUsage.useRealImag rec.Alg.parXT.PT.signalUsage.useComplex])<=1,'Type of Pilot Tone signal usage not internal consistent.');
...
if ~isfield(rec.Alg.parXT.PT.PTFilter,'medFiltKernelWidthms');rec.Alg.parXT.PT.PTFilter.medFiltKernelWidthms=0;end%Size of the median filter in ms used for removing outliers in the temporal PT signal.
...
if ~isfield(rec.Alg.parXT.PT.profileClustering,'Flag');rec.Alg.parXT.PT.profileClustering.Flag=0;end %Whether to use the PT-based binning.
if ~isfield(rec.Alg.parXT.PT.profileClustering,'numClusters');rec.Alg.parXT.PT.profileClustering.numClusters=100;end %The number of clusters to use for profile binning
