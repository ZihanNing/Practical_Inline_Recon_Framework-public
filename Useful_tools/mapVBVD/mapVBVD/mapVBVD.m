...
% http://undocumentedmatlab.com/blog/explicit-multi-threading-in-matlab-part1
    ...
    if isOctave % octave does not support a cancel button
        %h = waitbar(0,'','Name', sprintf('Reading Scan ID %d/%d', scan, Nscans));%YBwaitbarHandling
    else
        %h = waitbar(0,'','Name', sprintf('Reading Scan ID %d/%d', scan, Nscans),...
        %        'CreateCancelBtn',...
        %        'setappdata(gcbf,''canceling'',1)');
        %setappdata(h,'canceling',0)%YBwaitbarHandling
    end
    ...
    while true
        ...
        %if ~isOctave && getappdata(h,'canceling') %YBwaitbarHandling
        %    break;
        %end
        ...
        if progress > last_progress  + 0.01
            ...
            %waitbar(progress, h, progress_str);%YBwaitbarHandling
        end

        cPos = cPos + ulDMALength;
    end % while true
    %delete(h);%YBwaitbarHandling
