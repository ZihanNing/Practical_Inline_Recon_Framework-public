...
function [] = handle_connection_Bucket_blank(connection)
    ...
    while ReadNext_flag
        ...
        if bucketData.data.count == 1 
            ...
            imageref_bucket = bucketData; % ZN: copy all bucket here
    ...
    if ~isempty(noise_bucket) && loop_counter>1
     ...
     % twix_like = BucketToBuffer_matlab(bucketData,connection.header,noiseData); 
